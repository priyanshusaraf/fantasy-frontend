...
export default function ManageTournamentPage() {
  ...
  useEffect(() => {
    ...
    // Redirect from old URL pattern to new URL structure
    if (window.location.pathname.includes("/admin/manage-tournament")) {
      router.replace(`/admin/tournaments/${tournamentId}`);
      return;
    }
    ...
    const fetchTournamentData = async () => {
      try {
        ...
        if (!response.ok) {
          console.log(`Regular API fetch failed for tournament ${tournamentId} with status ${response.status}`);
          
          // Try our direct endpoint as a fallback
          console.log("Attempting direct fetch fallback...");
          const directResponse = await fetch(`/api/admin/direct-tournament-fetch?id=${tournamentId}`);
          
          if (!directResponse.ok) {
            throw new Error("Failed to fetch tournament details from both endpoints");
          }
          
          const directTournamentData = await directResponse.json();
          console.log("Direct fetch successful:", directTournamentData);
          setTournament(directTournamentData);
          
          // Since we're using the direct endpoint, we might not have stats
          // So let's create some minimal stats
          setStats({
            totalMatches: 0,
            completedMatches: 0,
            upcomingMatches: 0,
        ...
        if (statsResponse.ok) {
          ...
          console.log(`Regular stats API fetch failed with status ${statsResponse.status}`);
          
          // Try our direct endpoint as a fallback
          try {
            console.log("Attempting direct stats fetch...");
            const directStatsResponse = await fetch(`/api/admin/direct-tournament-stats?id=${tournamentId}`);
            
            if (directStatsResponse.ok) {
              const directStatsData = await directStatsResponse.json();
              console.log("Direct stats fetch successful:", directStatsData);
              setStats(directStatsData);
            } else {
              // If both fail, set default stats
              setStats({
                totalMatches: 0,
                completedMatches: 0,
                upcomingMatches: 0,
        ...
        if (playerStatsResponse.ok) {
          ...
          // If player stats fail, set empty array
          console.log(`Regular player stats API fetch failed with status ${playerStatsResponse.status}`);
          setPlayerStats([]);
        }
      } catch (err) {
        setError(
  ...
  const handleManagePlayers = () => {
    ...
    router.push(`/admin/player-management?tournamentId=${tournamentId}`);
  };
    ...
    return (
      <div className="container mx-auto px-4 py-8">
        <Alert variant="destructive" className="mb-4">
          ...
          <AlertDescription>
            {error || "Tournament data couldn't be loaded"}
          </AlertDescription>
        </Alert>
        ...
        <Card className="mb-4">
          <CardHeader>
            ...
            <CardDescription>
              The tournament data could not be loaded. This could be due to network issues,
              authentication problems, or the tournament may not exist.
            </CardDescription>
          </CardHeader>
          <CardContent>
                    ...
                    onClick={async () => {
                      try {
                        ...
                        if (directResponse.ok) {
                          ...
                          setPlayerStats([]);
                          setLoading(false);
                          toast.success("Tournament loaded via direct access");
                        } else {
                          throw new Error("Direct fetch failed");
                        }
                      } catch (directError) {
                        setError("All fetch attempts failed. Tournament may not exist.");
                        setLoading(false);
